...
mod robust_sum_test {
    ...
    #[test]
    fn test_robust_sum() {
        assert!(cmp(robust_sum(vec!(1., 64.0), vec!(-1e-64, 1e64)), vec!(-1e-64, 65., 1e64)));
        assert!(cmp(robust_sum(vec!(0.), vec!(0.)), vec!(0.)));
        assert!(cmp(robust_sum(vec!(0.), vec!(1.)), vec!(1.)));
        assert!(cmp(robust_sum(vec!(1., 1e64), vec!(1e-64, 2.)), vec!(1e-64, 3., 1e64)));
        assert!(cmp(robust_sum(vec!(1.), vec!(1e-64, 1e-16)), vec!(1e-64, 1e-16, 1.)));

        for i in -10..(10 + 1) {
            for j in -10..(10 + 1) {
                assert!(cmp(robust_sum(vec!(i as f64), vec!(j as f64)), vec!((i + j) as f64)));
            }
        }
